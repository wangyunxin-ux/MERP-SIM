...
namespace omnetpp {
thread_local simtime_t omnetpp::cSimulation::threadLocalSimTime = SIMTIME_ZERO;
...
// simtime_t_cref cSimulation::getSimTime() const
// {
//     return (threadLocalSimTime > currentSimtime) ? threadLocalSimTime : currentSimtime;
// }
...
void cSimulation::executeEvent(cEvent *event)
{
    ...
    thread_local bool inExecuteEvent = false;
    ...
    threadLocalSimTime= event->getArrivalTime();
    ...
    setGlobalContext();
    {
        std::lock_guard<std::mutex> lock(currentSimtimeMutex);
        // std::cout<<"currentSimtime="<<currentSimtime.dbl()<<"threadLocalSimTime="<<threadLocalSimTime.dbl()<<endl;
        if (threadLocalSimTime > currentSimtime) 
        {
            currentSimtime = threadLocalSimTime;
