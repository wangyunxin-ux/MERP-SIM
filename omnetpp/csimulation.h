...
class SIM_API cSimulation : public cNamedObject, noncopyable
{
  ...
  private:
    ...
    static thread_local bool inExecuteEvent;
    ...
    std::atomic<eventnumber_t> currentEventNumber{0}; // sequence number of current event
  ...
  public:
    ...
    void setGVT(simtime_t time){threadLocalSimTime = currentSimtime;}
    ...
    //  simtime_t_cref getSimTime() const  {return currentSimtime;}
     simtime_t_cref getSimTime() const  {return threadLocalSimTime;}
     simtime_t_cref getCurrentSimTime() const  {return currentSimtime;}
